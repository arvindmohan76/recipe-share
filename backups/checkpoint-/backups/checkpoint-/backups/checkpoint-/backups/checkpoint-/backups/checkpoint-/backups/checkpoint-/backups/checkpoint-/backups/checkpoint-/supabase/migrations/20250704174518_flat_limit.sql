/*
  # Fix Recommendations Generation System

  1. Functions
    - Drop and recreate recommendation generation function with better error handling
    - Drop and recreate analytics functions with proper return types
    - Add popular search terms function
...
BEGIN
    ...
    -- Check privacy settings (default to true if no settings exist)
    IF EXISTS (
...
BEGIN
    ...
    SELECT jsonb_build_object(
        ...
        'topSearchTerms', (
            SELECT COALESCE(jsonb_agg(jsonb_build_object('term', search_query, 'count', cnt)), '[]'::jsonb)
            FROM (
        ...
        'searchTrends', (
            SELECT COALESCE(jsonb_agg(jsonb_build_object('date', search_date, 'count', cnt)), '[]'::jsonb)
            FROM (
                SELECT 
